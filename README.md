...
普段のEdge・Excelなどの操作を記録し、操作前／操作後の画面とクリック位置をもとに、部内向けの操作マニュアルをローカルで組み立てるWindowsアプリです。手順名・説明・赤枠の候補は記録した事実からこのPC内で作るため、Microsoft 365 Copilotなどの外部AIは通常の作成導線で使いません。既存録画や画像からの作成も選べます。

主出力はExcel、必要な場合の副出力はWordとします。印刷用の厳密な組版より、PC上で画像と説明を同時に読みやすく、出力後も部内で追記・差し替えしやすいことを優先します。

## 現在の状態

- Phase 1基盤 v0.53.3: プロジェクト切替時に前の記録結果を破棄し、複数画像の取込進捗と取込後の完成UIを即時表示。画面差分だけの記録候補は記録対象名と最初の画面を確認するまで取り込めないように改善
- Phase 1基盤 v0.53.2: 操作イベントが0件でも、利用者が確認した画面差分候補を低信頼・赤枠なし・要確認の手順として取り込み、参照フレームと変換判断を操作証拠とは分けて保存
- Phase 1基盤 v0.53.1: 起動中のManualBuilderを `run.cmd` から開き直すときも、検証済みのセッショントークン付き入口URLを使い、画面本体認証の403へ落ちないように修正
...
- Phase 1基盤 v0.46.0: 記録直後はクリック・入力の時刻と安定フレームからローカル候補を先に表示し、Copilot精査は必要な場合だけバックグラウンドで選べる。遷移中・記録開始終了・根拠のないアプリ切替・入力途中の重複を除き、操作前／操作後と赤枠アンカーを分けて保持する。編集画面はカードを直接ドラッグし、Ctrl／Shiftで複数選択、削除と並べ替えを固定バーまたはCtrl+Zで取り消せる。Excel／Wordは4種類の前後画像配置、切り抜き、注釈、極端な縦横比を実Office全10ページで検証済み
- Phase 1基盤 v0.44.6: 操作記録を止めると、記録した事実からローカルで手順名・説明・赤枠を作り、そのまま編集へ進む。全手順の大きな画像をスクロールで見比べる一覧確認と、1件ずつの集中編集を切り替えられる。マニュアル削除は明確な確認後に完全削除し、ごみ箱やアーカイブの概念を持たない
- Phase 1基盤 v0.38.1: 市販の手順書作成ツールを参考にExcelの既定レイアウトを改善した。目次へ使い方案内を追加し、各シートでセクション番号・概要・総手順数を常時確認できるようにした。本文を「操作」、補足を「ポイント・注意」として視線誘導を明確にし、印刷時も1ページ幅、見出し繰り返し、ページ番号が保たれる
- Phase 1基盤 v0.38.0: 利用者ごとのローカルアプリへ製品方針を明確化した。HTMLファイル出力、共有フォルダーへの自動反映、配布HTMLからの自動取り込みを廃止し、Excelを標準出力、Wordを副出力とした。作業データはPC内に保ち、完成ファイルの共有・公開は利用者が明示的に行う
- Phase 1基盤 v0.37.4: 各手順へ操作後画像を自由に追加・差し替えでき、操作前のみ／操作後のみ／左右比較／上下比較と前後順を選択する。配置を画像の前で選べるようにし、狭い画面では選択肢と比較画像を読みやすく折り返す。編集画面で選んだ配置をExcel・Word出力まで維持する
- Phase 1基盤 v0.37.2: クリック前の操作箇所に加え、クリック後の安定した結果画面を同じ手順へ保存する。確認画面、Copilotの比較、編集画面、Excel・Word出力まで操作前／操作後を維持する
- Phase 1基盤 v0.37.0: アプリ起動と同時にCopilot用Edgeを非同期で準備する。画像の表示と処理完了、依頼文の末尾までを確認してから送信する。録画の代表画像は静止区間の多数派から選び、一瞬だけの差分候補や未精査の赤枠・拡大を除く
- Phase 1基盤 v0.36.4: Copilot反映後の「仕上げて出力」導線を追加した。全シートの説明・画像・赤枠・番号・要確認状態を集計し、該当シートへ移動できる。現在位置の直後への追加、個別・一括の削除と並べ替え、前後画像を移動しながらの注釈編集、出力前確認を1つの編集画面で行える。Copilotの不要候補や判断が曖昧な候補は自動削除せず、シートごとに利用者が確認できる
- Phase 1基盤 v0.36.1: 主導線を「操作を記録 → 使う操作を確認 → Copilotの提案を確認」へ変更した。専用EdgeプロファイルとCDP起動を廃止し、普段使っているEdge・Excel・エクスプローラーをUI Automation／MSAA／クリック位置で記録する。DOMなしでもクリック直前画像を保持し、Copilotが選べる赤枠候補と文章を人が画像上で確認してから反映できる。記録結果の誤破棄確認と狭い画面の表示も改善した
...
| --- | --- |
| OS | Windows 10 / 11 |
| PowerShell | Windows PowerShell 5.1（Windows標準。追加インストール不要） |
| Excel | **デスクトップ版が必須**（主出力）。Web版のみのPCではExcel出力できません |
| Word | デスクトップ版（副出力を使う場合のみ） |
| ブラウザー | Microsoft Edge（編集画面の表示と、記録モニターの表示に使います） |

外部AIサービス、APIキー、Microsoft Graphの権限、Microsoft 365 Copilotのライセンスは
いずれも不要です。作業データはPC内から出ません。

**Word出力の前にWordをすべて閉じてください。** Wordが1つでも起動していると、開いている
文書を壊さないために出力を開始せず中止します。

Excel出力は、Excelを開いたままでも作成用のExcelを別に起動して進みます。開いている
未保存のブックには触れません。ただし環境によっては、安全確認のために出力を中止して
「Excelを閉じて再実行してください」と表示することがあります。その場合はExcelを
すべて閉じてから、ダイアログの［もう一度作成］を押してください。

いずれの場合も、ManualBuilder側の入力内容は失われません。
...
- 起動時のマニュアル一覧から、新規作成、選択、名前検索、複製、削除
- マニュアル単位のZIP書き出しと、安全な新規項目としての取込み
- 文書タイトルの編集と自動保存
- シートの追加、切替、名称変更、ドラッグ並べ替え、削除
- 手順カードの追加、手順名・説明・補足の自動保存、削除
- 左の手順カードは、通常クリックで表示、`Ctrl`＋クリックで追加選択、`Shift`＋クリックで範囲選択。選択したカードを直接ドラッグしてまとめて並べ替え・別シート移動できる
- 手順またはシートにフォーカスして `Alt`＋`↑` `↓` を押すと、キーボードだけでも並べ替えられる
- 中央は全手順を大きな画像で縦に見比べる一覧確認が標準。必要に応じて選択した1手順だけの集中編集へ切り替えられる
...
- 「操作を記録」から、普段使っているEdge・Excel・エクスプローラーなどの操作を記録して手順を作る。専用プロファイルは使わず、WindowsのUI Automation／MSAAとクリック位置を赤枠候補として残す
...
- 記録を止めると、クリックの時刻・位置と画面変化からこのPC内で手順名・説明・赤枠の候補を作る。外部AIへは何も送らない
- 候補は確認画面で1件ずつ「この手順を使う」「この手順を除外」を選べる。除外した操作も、そのプロジェクト内には残る
- 動画つきの手順があるときは、Excelもブックと動画を1つのフォルダーへ出力し、見出し右の「▶ 動画を見る」から再生できる
- 完成したExcel／Wordの共有・公開は利用者が手動で行い、アプリは共有先を記憶したり自動更新したりしない

マニュアルごとのプロジェクトは `%LOCALAPPDATA%\ManualBuilder\data\projects\<マニュアルID>\project.json` に保存されます。
従来の `default` プロジェクトも一覧の最初のマニュアルとしてそのまま利用できます。マニュアルを削除すると、
確認後にプロジェクト、画像、動画を含む保存フォルダーを削除します。削除後は復元できません。
...
- 動画を見せたい場合はExcelを使う。Wordは動画を扱わず、静止画のまま出力する
- Excelは動画を埋め込めないが、動画つきの手順があるときだけブックと動画をフォルダーへまとめ、`=HYPERLINK()` の相対パスから再生できるようにする。動画が無い場合は単体のxlsxのままとし、メールで送れる手軽さを残す
- 作業データと出力は利用者のPC内に作る。アプリから共有フォルダーや公開先へ書き込まず、完成ファイルの配布は利用者が手動で行う
- HTMLはアプリの編集画面にだけ使い、マニュアル成果物としては出力しない
