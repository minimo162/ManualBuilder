...
- Phase 1基盤 v0.35.0: 操作記録をCopilot前提の方式へ変更した。記録用Edgeは起動せず、普段のEdge、エクスプローラー、業務アプリを同じ経路で記録する。各操作はクリック前後の原画像、クリック座標、UIA/MSAAの候補を証跡として保持し、候補を自動で赤枠へ確定しない。Microsoft 365 Copilotへ1操作ずつ前後・周辺画像を渡して、操作対象、操作の意味、手順文をまとめて解析する。送信前の黒塗りは手動のままとし、AIの不要判定や要確認判定も確認画面で復元・採用できる。合成画像による事前確認とイベント単位の結果保存も追加した
...
- その他メニューの「操作を記録して手順にする」から、普段使っているEdgeやデスクトップアプリの操作を記録する。取り込み後は、必要な黒塗りを手動で追加してから「Copilotで記録した操作を解析」を実行し、対象・赤枠・手順文を確認して確定する
...
Phase 1 v0.35.0の再確認手順は [docs/RETEST-PHASE1-v0.35.0.md](docs/RETEST-PHASE1-v0.35.0.md) にまとめています。
