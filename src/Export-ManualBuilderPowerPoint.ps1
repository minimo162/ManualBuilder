# ManualBuilder PowerPoint export worker.
...
Import-Module (Join-Path $PSScriptRoot 'ManualBuilder.PowerPoint.psm1') -Force

try {
    $project = Get-MbProject -Path $ProjectPath
    $result = Invoke-MbPowerPointExport -Project $project -ProjectPath $ProjectPath -OutputDirectory $OutputDirectory `
    ...
    $status = [pscustomobject]@{
        ...
        message = 'PowerPoint出力処理を開始できませんでした: ' + $_.Exception.Message
