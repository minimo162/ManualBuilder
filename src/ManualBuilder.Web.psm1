...
function ConvertTo-MbProjectLibraryHtml {
    ...
    $sb = New-Object System.Text.StringBuilder
    [void]$sb.AppendLine('<div id="workspace" class="workspace project-library" data-app-version="0.25.0">')
    ...
    [void]$sb.AppendLine('<footer class="project-library__footer"><span>v0.25.0</span></footer></main></div>')
    return $sb.ToString()
}
...
function ConvertTo-MbWorkspaceHtml {
    ...
    [void]$sb.AppendLine('<div id="workspace" class="workspace" data-app-version="0.25.0" data-revision="' + [int]$Project.revision + '" data-capture-version="' + $CaptureVersion + '">')
    ...
    [void]$sb.AppendLine('<div class="topbar__actions"><button type="button" class="button button--primary" data-export-excel>Excelで作成</button><details class="action-menu topbar-menu"><summary class="icon-button" title="その他" aria-label="その他の操作">…</summary><div class="action-menu__panel action-menu__panel--right"><button type="button" class="menu-command" data-export-powerpoint>PowerPointで作成（動画つき）</button><button type="button" class="menu-command" data-export-html>HTMLで作成（ブラウザー用）</button><button type="button" class="menu-command" data-export-word>Wordで作成</button><button type="button" class="menu-command menu-command--danger" hx-post="/api/shutdown" hx-target="body" hx-swap="none" hx-confirm="ManualBuilderを終了しますか？">ManualBuilderを終了</button></div></details></div>')
    [void]$sb.AppendLine('</header>')

    [void]$sb.AppendLine('<div class="app-layout">')
    [void]$sb.AppendLine('<aside class="sidebar">' + (Render-MbSheetNavigation -Project $Project) + (Render-MbStepNavigation -Sheet $sheet) + '<div class="sidebar__footer"><span class="sidebar__version">v0.25.0</span></div></aside>')
