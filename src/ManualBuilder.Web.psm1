...
function ConvertTo-MbSaveStatusHtml {
    ...
    return '<span id="save-status" class="save-status save-status--' + $State + '" role="status"><span aria-hidden="true">' + $icon + '</span><span class="save-status__message">' + (ConvertTo-MbHtml ($Message + $time)) + '</span></span>'
}
...
function Render-MbSheetNavigation {
    ...
    foreach ($sheet in @($Project.sheets)) {
        ...
        [void]$sb.AppendLine('<div class="sheet-nav__item' + $active + '" data-sheet-direct-drag data-sheet-nav-item data-sheet-id="' + $sheetId + '" data-sheet-drop-target>')
        [void]$sb.AppendLine('<button type="button" class="sheet-nav__main" hx-post="/api/sheets/select" hx-vals=''{"sheetId":"' + $sheetId + '"}'' hx-target="#workspace" hx-swap="outerHTML"' + $current + '><span class="sheet-nav__name">' + $name + '</span><span class="sheet-nav__count">' + $stepCount + '</span></button>')
...
function Render-MbStepNavigation {
    ...
    $steps = @($Sheet.steps)
    $sb = New-Object System.Text.StringBuilder
    [void]$sb.AppendLine('<nav class="step-nav" aria-label="選択中のシートの手順">')
    [void]$sb.AppendLine('<div class="step-nav__heading"><span>手順 <span class="count-badge">' + $steps.Count + '</span></span></div>')
    if ($steps.Count -gt 1) {
        [void]$sb.AppendLine('<div class="step-nav__guide" data-step-sort-guide aria-live="polite">手順をつかんで並べ替え。Ctrl・Shiftで複数選択。Alt＋↑↓でも移動</div>')
    }
...
function ConvertTo-MbStepCardHtml {
    ...
    # 手順カードに見出しを与える。以前は文書内の見出しが sr-only の h1 だけで、
    # 読み上げ利用者はHキーで手順を渡り歩けず、20手順を頭から読み下すしかなかった。
    [void]$sb.AppendLine('<article class="step-card' + $imageStateClass + '" id="step-' + $stepId + '" data-step-id="' + $stepId + '" aria-labelledby="step-heading-' + $stepId + '">')
    ...
    [void]$sb.AppendLine('<h2 class="step-card__label" id="step-heading-' + $stepId + '">手順 ' + $Number + '</h2>')
    [void]$sb.AppendLine('<button type="button" class="step-card__edit" data-step-edit>この手順を編集</button>')
    [void]$sb.AppendLine('</div>')
    ...
    if ($Step.imageId) {
        ...
        $addResultButton = if (-not $hasResultImage) { '<button type="button" class="image-secondary-button image-secondary-button--compare" data-add-result-image>結果画像を追加</button>' } else { '' }
        ...
        if ($hasResultImage) {
            ...
            if ($imageLayout -eq 'before') {
                [void]$sb.AppendLine('<div class="result-image-available"><div><strong>操作後の結果も保存されています</strong><span>必要な手順だけ、2枚の画像で変化を説明できます</span></div><button type="button" class="image-secondary-button" data-image-layout-option="side-by-side">結果画像も表示</button></div>')
            } else {
                $layoutLabels = [ordered]@{ before = '案内画像だけ'; after = '結果画像だけ'; 'side-by-side' = '2枚を横に並べる'; stacked = '2枚を縦に並べる' }
                [void]$sb.AppendLine('<div class="image-layout-editor"><div class="image-layout-editor__heading"><strong>2枚の画像の見せ方</strong><span>選んだ配置でマニュアルに出力します</span></div><div class="image-layout-options" role="radiogroup" aria-label="2枚の画像の見せ方">')
                ...
                [void]$sb.AppendLine('</div><div class="image-layout-editor__actions"><button type="button" class="image-secondary-button" data-open-annotation data-image-edit-target="result">結果画像を編集' + $resultAnnotationBadge + $resultCropBadge + '</button><button type="button" class="image-secondary-button" data-swap-image-order>2枚の順序を入れ替える</button><button type="button" class="image-secondary-button" data-replace-result-image>結果画像を差し替え</button><button type="button" class="image-secondary-button image-secondary-button--danger" data-remove-result-image>結果画像を外す</button></div></div>')
            }
        }
        ...
        [void]$sb.AppendLine('<div class="step-visual-item step-visual-item--before"><span class="step-visual-item__label">操作する場所</span><div class="step-image-frame"><button type="button" class="step-image-button" data-image-preview="' + $imageUrl + '" data-image-preview-kind="before" aria-label="手順 ' + $Number + ' の案内画像を拡大"><span class="step-image-viewport"><img class="step-image" src="' + $imageUrl + '" alt="手順 ' + $Number + ' の操作する場所" loading="lazy"><svg class="step-annotation-overlay" viewBox="0 0 1000 1000" preserveAspectRatio="none" aria-hidden="true"></svg></span></button></div></div>')
        if ($hasResultImage) {
            $resultImageId = ConvertTo-MbHtml ([string]$Step.resultImageId)
            $resultImageUrl = '/images/' + $resultImageId + '?token=' + (ConvertTo-MbHtml $Token)
            [void]$sb.AppendLine('<div class="step-visual-item step-visual-item--after"><span class="step-visual-item__label">操作後の結果</span><div class="step-result-image"><button type="button" class="step-result-image__button" data-image-preview="' + $resultImageUrl + '" data-image-preview-kind="result" aria-label="手順 ' + $Number + ' の結果画像を拡大"><span class="step-result-image__viewport"><img class="step-result-image__image" src="' + $resultImageUrl + '" alt="手順 ' + $Number + ' の操作後の結果" loading="lazy"><svg class="step-annotation-overlay step-result-annotation-overlay" data-image-edit-target="result" viewBox="0 0 1000 1000" preserveAspectRatio="none" aria-hidden="true"></svg></span></button></div></div>')
        }
        [void]$sb.AppendLine('</div>')
        [void]$sb.AppendLine('<div class="image-edit-actions" aria-label="画像の操作"><button type="button" class="image-edit-button" data-open-annotation data-image-edit-target="before" title="赤枠・番号・切り抜き・矢印・黒塗り"><span class="image-edit-button__icon" aria-hidden="true">＋</span><span class="image-edit-button__copy"><strong>' + $annotationActionTitle + '</strong><span>' + $annotationActionHint + '</span></span>' + $annotationBadge + $cropBadge + '</button><div class="image-secondary-actions">' + $removeFocusRectButton + '<button type="button" class="image-secondary-button" data-replace-image>案内画像を差し替え</button>' + $undoButton + $addResultButton + '</div>' + $videoRow + '</div>')
...
function ConvertTo-MbWatchStatusHtml {
    ...
    $icon = if ($State -eq 'active' -and $Role -eq 'owner') { '●' } elseif ($State -eq 'standby') { '◐' } else { '○' }
    $detail = if ($State -eq 'disabled') {
        '貼り付け・ドロップ・画像選択で追加できます。'
    } elseif ($Directory) {
        'スクリーンショット保存先: ' + $Directory
    } else {
        '保存先を検出できないため、貼り付け・ドロップ・画像選択を利用できます。'
    }
...
function ConvertTo-MbProjectLibraryHtml {
    ...
    $sb = New-Object System.Text.StringBuilder
    [void]$sb.AppendLine('<div id="workspace" class="workspace project-library" data-app-version="0.53.2">')
    [void]$sb.AppendLine('<header class="topbar project-library__topbar"><button type="button" class="brand brand--home" data-project-home hx-post="/api/projects/home" hx-target="#workspace" hx-swap="outerHTML" title="マニュアル一覧" aria-label="マニュアル一覧" aria-current="page"><span class="brand__mark" aria-hidden="true">M</span><span>ManualBuilder</span></button><div class="project-library__topbar-title">マニュアル一覧</div><div></div><div class="topbar__actions"><details class="action-menu topbar-menu"><summary role="button" aria-expanded="false" class="icon-button" title="その他" aria-label="その他の操作を開く">…</summary><div class="action-menu__panel action-menu__panel--right"><button type="button" class="menu-command" data-import-project-package>ZIPからマニュアルを取り込む</button><button type="button" class="menu-command menu-command--danger" hx-post="/api/shutdown" hx-target="body" hx-swap="none" hx-confirm="ManualBuilderを終了しますか？">ManualBuilderを終了</button></div></details></div></header>')
    [void]$sb.AppendLine('<main class="project-library__main">')
    [void]$sb.AppendLine('<section class="project-library__intro"><div><p class="eyebrow">操作を記録して手順書を作る</p><h1>作業するマニュアルを選ぶ</h1><p class="project-library__lead">新しく作るか、下の一覧から続きのマニュアルを開きます。</p></div><div class="project-library__actions"><input id="project-package-input" type="file" accept=".zip,application/zip" hidden><form class="project-create" hx-post="/api/projects/create" hx-target="#workspace" hx-swap="outerHTML"><label for="new-project-title">新しいマニュアル名</label><div class="project-create__controls"><input id="new-project-title" type="text" name="title" maxlength="100" placeholder="例：経費申請の手順" aria-describedby="new-project-title-help"><button type="submit" class="button button--primary">新しいマニュアルを作る</button></div><small id="new-project-title-help">名前を空欄にすると「新しいマニュアル」で作成します。</small></form></div></section>')
    ...
    if (@($Projects).Count -eq 0) {
        [void]$sb.AppendLine('<div class="project-library__empty"><strong>マニュアルはまだありません</strong><span>上の「新しいマニュアル名」から作成できます。</span></div>')
    } else {
        ...
        foreach ($entry in @($Projects)) {
            ...
            if ([bool]$entry.readable) {
                ...
                [void]$sb.AppendLine('<div class="project-card__error">このマニュアルのデータを読み取れませんでした。復旧できない場合は、［…］から削除して作り直してください。<details class="project-card__error-detail"><summary>詳しい状況</summary><p>' + (ConvertTo-MbHtml $entry.error) + '</p></details></div></article>')
            }
        }
    ...
    [void]$sb.AppendLine('<footer class="project-library__footer"><span>v0.53.2</span></footer></main></div>')
    return $sb.ToString()
}
...
function ConvertTo-MbWorkspaceHtml {
    ...
    [void]$sb.AppendLine('<div id="workspace" class="workspace step-view--review" data-app-version="0.53.2" data-revision="' + [int]$Project.revision + '" data-capture-version="' + $CaptureVersion + '"><a class="skip-link" href="#editor-main">編集画面へスキップ</a>')
    ...
    # 撮影フォルダー監視は補助的な内部状態で、操作記録の自動画面取得と無関係。
    # ヘッダーへ出すと「画像は手動」と誤解されるため、利用者には保存状態だけを示す。
    [void]$sb.AppendLine('<div class="topbar__state">' + (ConvertTo-MbSaveStatusHtml) + '</div>')
    # 表示の出し分けは「今のシート」ではなくプロジェクト全体の手順数で判断する。
    # シートの数え方にすると、作業中の人がシートを1つ足しただけで初回案内へ戻り、
    # 「操作を記録」も作成メニューも消えて、その他メニューが終了だけになる。
    $topbarRecordAction = if ($projectStepCount -gt 0) { '<button type="button" class="button button--secondary topbar__main-action topbar__record-action" data-record-operations>操作を記録</button>' } else { '' }
    # 出力の入口は仕上げ欄の1つに絞る。同じ文言のボタンが上下に並ぶと押し分けの理由が読めず、
    # 画面内の主ボタンも「操作を記録」と2つになっていた。
    $topbarExportAction = ''
    $otherCreationActions = if ($projectStepCount -gt 0) { '<button type="button" class="menu-command" data-open-video-picker>録画ファイルを取り込む</button><button type="button" class="menu-command" data-open-image-picker>画像から手順を追加</button><button type="button" class="menu-command" hx-post="/api/steps/add" hx-vals=''{"sheetId":"' + $sheetId + '"}'' hx-target="#workspace" hx-swap="outerHTML">空の手順を追加</button>' } else { '' }
    $topbarMenu = if ([string]::IsNullOrWhiteSpace($otherCreationActions)) {
        # 中身が終了だけのメニューは出さない。初めて開くメニューの唯一の選択肢が
        # 「終了」だと、探索の手が止まるか誤操作を誘う。終了は一覧画面にもある。
        ''
    } else {
        '<details class="action-menu topbar-menu"><summary role="button" aria-expanded="false" class="icon-button" title="その他" aria-label="その他の操作を開く">…</summary>' +
            '<div class="action-menu__panel action-menu__panel--right">' + $otherCreationActions +
            '<button type="button" class="menu-command menu-command--danger" hx-post="/api/shutdown" hx-target="body" hx-swap="none" hx-confirm="ManualBuilderを終了しますか？">ManualBuilderを終了</button></div></details>'
    }
    [void]$sb.AppendLine('<div class="topbar__actions">' + $topbarRecordAction + $topbarExportAction + $topbarMenu + '</div>')
    [void]$sb.AppendLine('</header>')

    [void]$sb.AppendLine('<div class="app-layout">')
    [void]$sb.AppendLine('<aside class="sidebar">' + (Render-MbSheetNavigation -Project $Project) + (Render-MbStepNavigation -Sheet $sheet -Project $Project) + '<div class="sidebar__footer"><span class="sidebar__version">v0.53.2</span></div></aside>')
    [void]$sb.AppendLine('<main id="editor-main" class="editor" tabindex="-1">')
    ...
    [void]$sb.AppendLine('<div class="sheet-heading"><input type="hidden" name="sheetId" value="' + $sheetId + '"><label class="editable-name editable-name--sheet name-field" data-editable-name title="シートの名前を編集"><span class="sr-only">シートの名前</span><input class="sheet-name-input" type="text" name="name" maxlength="50" value="' + $sheetName + '" aria-label="シートの名前。入力して変更" hx-post="/api/sheets/rename" hx-trigger="input changed delay:700ms, change" hx-include="closest .sheet-heading" hx-target="#save-status" hx-swap="outerHTML"></label><span class="step-total">' + $steps.Count + ' 手順</span></div>')
    ...
    [void]$sb.AppendLine('<details class="action-menu sheet-menu"><summary role="button" aria-expanded="false" class="icon-button" title="シートのメニュー" aria-label="シートのメニューを開く">…</summary><div class="action-menu__panel action-menu__panel--right"><button type="button" class="menu-command" hx-post="/api/steps/add" hx-vals=''{"sheetId":"' + $sheetId + '"}'' hx-target="#workspace" hx-swap="outerHTML">空の手順を追加</button><button type="button" class="menu-command" data-sheet-duplicate data-sheet-id="' + $sheetId + '">シートを複製</button>')
    ...
    if ($steps.Count -gt 0) {
        [void]$sb.AppendLine('<section class="step-review-toolbar" data-step-review-toolbar aria-labelledby="step-review-title"><div class="step-review-toolbar__heading"><strong id="step-review-title">手順を確認・修正</strong><span>画面と説明を上から順に確認します</span></div><div class="step-view-options" role="group" aria-label="手順の表示方法"><button type="button" class="step-view-option" data-step-view="review" aria-pressed="true">一覧で確認</button><button type="button" class="step-view-option" data-step-view="focus" aria-pressed="false">1件ずつ編集</button></div><div class="step-review-navigation" aria-label="手順の移動"><button type="button" data-step-previous aria-label="前の手順">← 前</button><span data-step-position aria-live="off">1 / ' + $steps.Count + '</span><button type="button" data-step-next aria-label="次の手順">次 →</button></div><button type="button" class="button button--primary step-review-toolbar__export" data-open-export-dialog>Excelで作成</button></section>')
    }

    if ($projectStepCount -gt 0) {
        [void]$sb.AppendLine('<section class="finish-guide" data-finish-guide aria-labelledby="finish-guide-title"><div class="finish-guide__heading"><span class="finish-guide__mark" aria-hidden="true">✓</span><div><strong id="finish-guide-title">出力前の確認</strong><span data-finish-summary>説明と画像を確認します</span></div></div><div class="finish-guide__checks" aria-label="出力前の確認"><button type="button" data-finish-check="text"><span>説明</span><strong data-finish-text>確認中</strong></button><button type="button" data-finish-check="image"><span>画像</span><strong data-finish-image>確認中</strong></button><button type="button" data-finish-check="attention"><span>要確認</span><strong data-finish-attention>なし</strong></button></div></section>')
    }
    ...
    if ($steps.Count -eq 0 -and $projectStepCount -gt 0) {
        # 他のシートに手順がある人は初回ではない。初回案内ではなく、このシートを埋める手段だけを示す。
        [void]$sb.AppendLine('<div class="empty-state empty-state--sheet drop-target"><div class="empty-state__icon empty-state__icon--record" aria-hidden="true">●</div><h2>このシートはまだ空です</h2><p class="empty-state__lead">操作を記録するか、手順を追加してこのシートを作ります。</p><div class="empty-state__actions"><button type="button" class="button button--primary empty-state__main-button" data-record-operations>操作を記録</button><button type="button" class="button button--secondary empty-state__button" data-open-video-picker>録画ファイルを取り込む</button><button type="button" class="button button--ghost empty-state__button" data-open-image-picker>画像から作る</button><button type="button" class="button button--ghost empty-state__button" hx-post="/api/steps/add" hx-vals=''{"sheetId":"' + $sheetId + '"}'' hx-target="#workspace" hx-swap="outerHTML">空の手順を追加</button></div></div>')
    } elseif ($steps.Count -eq 0) {
        [void]$sb.AppendLine('<div class="empty-state drop-target"><p class="empty-state__eyebrow">初めての方はこちら</p><div class="empty-state__icon empty-state__icon--record" aria-hidden="true">●</div><h2>操作を記録して、手順書を作る</h2><p class="empty-state__lead">小さい操作画面を開き、対象アプリへ移動してから記録を始めます。移動中の操作は記録されません。</p><ol class="empty-state__flow" aria-label="作成の流れ"><li><span>1</span>対象アプリで記録</li><li><span>2</span>手順を確認・修正</li><li><span>3</span>Excelで作成</li></ol><button type="button" class="button button--primary empty-state__main-button" data-record-operations>記録の操作画面を開く</button><p class="empty-state__privacy">画像と操作情報はこのPC内だけに保存されます。</p><details class="empty-state__alternatives"><summary>別の方法で作る</summary><div class="empty-state__actions"><button type="button" class="button button--secondary empty-state__button" data-open-video-picker>録画ファイルを取り込む <small>mp4・webm</small></button><button type="button" class="button button--ghost empty-state__button" data-open-image-picker>画像から作る</button><button type="button" class="button button--ghost empty-state__button" hx-post="/api/steps/add" hx-vals=''{"sheetId":"' + $sheetId + '"}'' hx-target="#workspace" hx-swap="outerHTML">空の手順を追加</button></div><p>録画や画像を使わず、文章だけの手順も作れます。</p></details></div>')
    } else {
        for ($i = 0; $i -lt $steps.Count; $i++) {
            $canUndo = $UndoImageStepIds -contains [string]$steps[$i].id
            $stepVideo = Get-MbStepVideoEntry -Project $Project -Step $steps[$i]
