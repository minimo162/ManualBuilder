...
function Render-MbStepNavigation {
    ...
    $selectAllDisabled = if ($steps.Count -eq 0) { ' disabled' } else { '' }
    [void]$sb.AppendLine('<div class="step-nav__heading"><span class="step-nav__heading-title"><span>手順</span><span class="count-badge">' + $steps.Count + '</span></span><label class="step-nav__select-all"><input type="checkbox" data-step-select-all aria-label="このシートの手順をすべて選択"' + $selectAllDisabled + '><span>すべて選択</span></label></div>')
...
function ConvertTo-MbStepCardHtml {
    ...
    if ($Step.capture -and $Step.capture.PSObject.Properties.Name -contains 'analysisState' -and
        [string]$Step.capture.analysisState -in @('pending', 'needs-review')) {
        $analysisBadge = '<span class="step-card__pending">Copilot解析待ち</span>'
    }
    ...
    [void]$sb.AppendLine('<span class="step-card__label">手順 ' + $Number + '</span>' + $analysisBadge)
    [void]$sb.AppendLine('</div>')
...
function ConvertTo-MbProjectLibraryHtml {
    ...
    [void]$sb.AppendLine('<footer class="project-library__footer"><span>v0.35.0</span></footer></main></div>')
    return $sb.ToString()
}
...
function ConvertTo-MbWorkspaceHtml {
    ...
    [void]$sb.AppendLine('<div id="workspace" class="workspace" data-app-version="0.35.0" data-revision="' + [int]$Project.revision + '" data-capture-version="' + $CaptureVersion + '">')
    ...
    [void]$sb.AppendLine('<div class="topbar__actions"><button type="button" class="copilot-runtime copilot-runtime--starting" data-copilot-runtime data-state="starting" title="クリックしてCopilot画面を開く"><span class="copilot-runtime__dot" aria-hidden="true"></span><span data-copilot-runtime-label>Copilot準備中</span></button><button type="button" class="button button--primary" data-export-excel>Excelで作成</button><button type="button" class="button button--secondary" data-export-html>HTMLで作成</button><details class="action-menu topbar-menu"><summary class="icon-button" title="その他" aria-label="その他の操作">…</summary><div class="action-menu__panel action-menu__panel--right"><button type="button" class="menu-command" data-record-operations>操作を記録して手順にする</button><button type="button" class="menu-command" data-copilot-operation>Copilotで記録した操作を解析</button><button type="button" class="menu-command" data-copilot-draft>Copilotで手順の文章を作る</button><button type="button" class="menu-command" data-copilot-review>Copilotで文章を整える</button><button type="button" class="menu-command" data-export-word>Wordで作成</button><button type="button" class="menu-command menu-command--danger" hx-post="/api/shutdown" hx-target="body" hx-swap="none" hx-confirm="ManualBuilderを終了しますか？">ManualBuilderを終了</button></div></details></div>')
    [void]$sb.AppendLine('</header>')

    [void]$sb.AppendLine('<div class="app-layout">')
    [void]$sb.AppendLine('<aside class="sidebar">' + (Render-MbSheetNavigation -Project $Project) + (Render-MbStepNavigation -Sheet $sheet -Project $Project) + '<div class="sidebar__footer"><span class="sidebar__version">v0.35.0</span></div></aside>')
